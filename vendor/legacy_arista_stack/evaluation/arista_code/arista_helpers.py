...
def load_models(
    ...
    exp_dir = os.path.join(RES_DIR, exp_name or config["exp"]["name"])
    if not os.path.isdir(exp_dir):
        raise FileNotFoundError(f"Legacy model directory not found: {exp_dir}")
...
def train_mlp_classifier(
    ...
    *,
    ...
    force_load_cache_path: Optional[str] = None,
    ...
    def _try_load_cached(meta_expected: _ClassifierCacheMeta, cache_path_in: str):
        ...
        try:
            ...
            force_load = bool(
                force_load_cache_path
                and os.path.abspath(cache_path_in) == os.path.abspath(force_load_cache_path)
            )
            if (not force_load) and meta_loaded.to_dict() != meta_expected.to_dict():
                return None

            model_loaded = ResidualMLP(
                input_size=int(meta_loaded.input_size or meta_expected.input_size),
                hidden_size=int(meta_loaded.hidden_size or meta_expected.hidden_size),
                num_classes=len(meta_loaded.classes or meta_expected.classes),
            )
            model_loaded.load_state_dict(payload["state_dict"])
            model_loaded.eval()

            label_encoder_loaded = LabelEncoder()
            label_encoder_loaded.classes_ = np.asarray(list(meta_loaded.classes or meta_expected.classes))
            acc_loaded = payload.get("acc", None)
            if progress:
                try:
                    prefix = "force-loaded" if force_load else "loaded"
                    print(f"[train_mlp_classifier] {prefix} cache: {cache_path_in}")
                except Exception:
                    pass
    ...
    if resolved_cache_path and (cache_dir or cache_path):
        try:
            if force_load_cache_path and os.path.abspath(resolved_cache_path) == os.path.abspath(force_load_cache_path):

...
try:
    from DeepRUOT.interaction import cal_interaction as legacy_cal_interaction
    _USE_LEGACY_INTERACTION = True
except Exception:
    legacy_cal_interaction = None
    _USE_LEGACY_INTERACTION = False
    from CytoBridge.tl.core.interaction import cal_interaction as cytobridge_cal_interaction


def _compute_interaction_force(
    data_tensor,
    lnw,
    interaction_net,
    timepoint_idx: float,
    ...
    if _USE_LEGACY_INTERACTION:
        return legacy_cal_interaction(
            data_tensor,
            lnw,
            interaction_net,
    ...
    return cytobridge_cal_interaction(
        data_tensor,
        lnw,
        interaction_net,
        m=interaction_m,
        cutoff=interaction_threshold,
        t=torch.tensor([timepoint_idx], dtype=torch.float32, device=device),
    )
...
def palette_to_sequence(adata, color_key: str, palette: Optional[Dict[str, str] | Sequence[str]]):
    """Normalize palettes for Scanpy/scVelo plotting APIs."""
    if palette is None or not isinstance(palette, dict):
        return palette
    if color_key not in adata.obs:
        return palette
    ...
    if hasattr(series, "cat"):
        ...
        categories = [str(x) for x in pd.Categorical(series.astype(str)).categories.tolist()]
    return [palette.get(cat, palette.get(str(cat), "#808080")) for cat in categories]
...
def plot_single_velocity_field(
    ...
    **kwargs,
    ...
    palette_arg = palette_to_sequence(adata_plot, color_key, palette)
...
def velocity_fingerprint_stream_simple(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        interaction_full = _compute_interaction_force(
            data_tensor,
            lnw,

...
def simulate_sde_points(
    ...
    import numpy as np
    import torch
    from CytoBridge.tl.core.interaction import cal_interaction, euler_sdeint
    ...
    class SDE(torch.nn.Module):
        ...
        def f(self, t, y):
            ...
            with torch.no_grad():
                ...
                dlnw = self.g_net(t, z)
                net_forces = cal_interaction(z, lnw, self.interaction, m=interaction_m, t=t)
            if include_score:
...
def simulate_sde_points_split(
    ...
    from CytoBridge.tl.downstream.analysis import euler_sdeint_split
...
def compute_velocity_components(
    ...
    elif hasattr(score_net, "compute_gradient"):
        score_grad = score_net.compute_gradient(t_tensor, data_tensor)
    else:
        raise TypeError("score_net must be callable or expose compute_gradient(t, x)")
...
def compute_drift(df, dim: int, f_net, score_net, interaction_m: int = 1024, device: str = "cuda"):
    ...
    if callable(score_net):
        ...
        gradients_score = score_net.compute_gradient(t_tensor, data_tensor)
    else:
        raise TypeError("score_net must be callable or expose compute_gradient(t, x)")

...
3. `recovered_six_stage_fixed_rbf_historical_double_transform_auto_thresholds`:
   the historical notebook path, which normalized/log1p-transformed an H5AD
   `.X` already numerically equivalent to `log1p(layers['counts'])`, then used
   the same training implementation with automatically selected graph thresholds;
...
combined effect of the historical double-transformation/alignment path,
automatically selected thresholds, and the edge-classifier fit; they are not a
single-factor threshold comparison. The strict 3→4 comparison freezes both aligned H5AD and
...
## Historical four-condition full-data audit
...
| recovered six-stage, historical double-transform + automatic thresholds | 3.11900 | 3.15910 | 3.05329 | 0.06648 | 0.08182 | 0.00624 | 0.8966 | 0.8753 | 0.8882 | 0.0127 |
...
local spatial collapse. Condition 3 is retained as a historical preprocessing
audit, not as the clean selected retrain; the published checkpoint remains the
saved-model reference until the counts-source rerun is compared below. The
historical/current median nearest-neighbor distances
...
| --- | --- | --- | --- |
| 5a | Observed/generated stacked spatial map, reaEGC-focused lineage trajectories, and directed interaction edges | `scripts/run_arista_formal_panels_api.py` | Formal rerun uses 7,668 particles and no spatial warp. Persistent ribbons/lineage come from the non-split SDE; displayed clouds and recomputed attention communication come from the split SDE. Classifier KNN is `k=1`. |
| 5b | Generated 3.5-DPI spatial slice | `scripts/run_arista_formal_panels_api.py` | Generated no-warp t=0.5 model-time slice, corresponding to 3.5 DPI after dataset-time relabeling |
...
| --- | --- | --- | --- |
| S12 | Observed and generated spatial maps on the dense 0.5-step grid | `scripts/run_arista_formal_panels_api.py` | Shared no-warp interpolation run; observed/generated snapshots and the mosaic are exported under `snapshots/` |
| S13 | Growth-rate maps on the dense time grid | `scripts/run_arista_formal_panels_api.py` calling `evaluate_growth_by_timepoint` + `plot_growth_timepoint_grid` | Per-cell `growth_dense_time_grid.csv` and the dense-grid SVG are exported. Any injury marker must be supplied as explicit dataset metadata rather than inferred silently. |
| S14a | Lineage Sankey across observed and interpolated stages | `scripts/run_arista_formal_panels_api.py` calling `plot_lineage_sankey` | Non-split fixed particles preserve row identity across stages; the split population is not misused as persistent lineages. |
| S14b | Cell-type composition across observed and interpolated stages | `scripts/run_arista_formal_panels_api.py` calling `summarize_label_composition` + `plot_celltype_composition` | Long-form table and manuscript-style stacked-bar SVG are emitted from the same classifier output as S14a. |
| S15 | Reconstructed temporal gene patterns and GO enrichment | `summarize_temporal_gene_patterns`, PCA inverse reconstruction, temporal clustering, `overrepresentation_analysis`, and the package enrichment plots | **Full run complete.** All 2,000 genes are clustered separately from the display-only top genes, and GO BP 2023 is recomputed offline from a hashed GMT. Published-checkpoint recomputation gives 591/1,409 genes with 7/78 significant terms; selected retraining gives 792/1,208 with 6/97. Against the frozen 511/1,489 split, best-matched agreement/ARI are 0.949/0.799 and 0.845/0.470, respectively. The fixed GMT tests a 14,698-gene universe; the archived clusterProfiler BP table used 18,870, so the frozen table is retained as a database-version control and can be redrawn through the current plotting API. |
